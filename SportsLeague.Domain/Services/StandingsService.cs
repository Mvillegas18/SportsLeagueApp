...

{
...

    {
        ...
        var allResults = new List<MatchResult>(); foreach (var matchId in matchIds)
...

    {
        ...
        var matchIds = matches.Select(m => m.Id).ToHashSet();    // Obtener goles de esos partidos 
...

    {
        ...
        if (tournament == null)
            ...
            throw new KeyNotFoundException(
                ...
                $"No se encontró el torneo con ID {tournamentId}"); var matches = await _matchRepository.GetByTournamentAsync(tournamentId);
